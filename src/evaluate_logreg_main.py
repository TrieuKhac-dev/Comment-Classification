"""
Entry point để đánh giá (evaluate) Logistic Regression model đã train.
...
Usage:
    ...
    python -m src.evaluate_logreg_main --data data/raw/test.xlsx
    
    # Mode 2: Evaluate với processed data file (CSV)
    python -m src.evaluate_logreg_main --processed data/processed/processed_logreg_YYYYMMDD_test_xxx.csv
    
    # Evaluate với cache prefix tùy chỉnh
    python -m src.evaluate_logreg_main --data data/raw/test.xlsx --use-cache --cache-prefix my_eval
"""
...
from src.models.classifier.base_classifier_context import (
...
def evaluate_with_raw_data(
    ...
    """
    Đánh giá Logistic Regression model với file data mới (test trên TOÀN BỘ samples, không split).
    """
    logger_service = container.resolve("logger_service")
    logger_service.info(
        "=== [LogReg] Evaluate model with raw data (no split, test on all samples) ==="
    )
    ...
    if use_cache and not feature_cache_prefix:
        ...
        feature_cache_prefix = f"eval_logreg_{timestamp}"
    ...
    pipeline = LogRegPipelineConfig.evaluation_pipeline_no_split(
    ...
    logger_service.info("=== [LogReg] Evaluation completed ===")

    # In kết quả
    if hasattr(context, "metrics") and context.metrics and "evaluate" in context.metrics:
        print("\n" + "=" * 60)
        print("EVALUATION RESULTS - LogReg - Full Dataset")
        print("=" * 60)
...
def evaluate_with_processed_data(
    ...
    """
    Đánh giá Logistic Regression model với processed data file (đã qua preprocessing - CSV UTF-8).
    """
    logger_service = container.resolve("logger_service")
    logger_service.info("=== [LogReg] Evaluate model with processed data ===")
    logger_service.info(f"Model: {model_path}")
    ...
    context: BaseClassifierContext = builder.build()

    # Tạo pipeline evaluate với processed data
    pipeline = LogRegPipelineConfig.evaluation_pipeline_with_processed(
    ...
    if hasattr(context, "metrics") and context.metrics and "evaluate" in context.metrics:
        print("\n" + "=" * 60)
        print("EVALUATION RESULTS - LogReg - Processed Dataset")
        print("=" * 60)
        for metric, value in context.metrics["evaluate"].items():
            print(f"  {metric:15s}: {value:.4f}")
        print("=" * 60 + "\n")
...
def main():
    """Entry point chính với argument parsing cho Logistic Regression."""
    parser = argparse.ArgumentParser(
        description="Evaluate Logistic Regression model with raw data or processed data",
        ...
        python -m src.evaluate_logreg_main --processed data/processed/processed_logreg_YYYYMMDD_test_abc123.csv
        
        # Evaluate với cache features
        python -m src.evaluate_logreg_main --data data/raw/test.xlsx --use-cache
    ...
    parser.add_argument(
        "--model",
        type=str,
        help="Model path (default: latest Logistic Regression model)",
    )
    parser.add_argument(
        "--use-cache",
        action="store_true",
    ...
    if not args.model:
        ...
        if not model_path:
            print(f"Error: No Logistic Regression model found in {paths.LOGREG_DIR}")
            sys.exit(1)
        print(f"Auto-detected latest Logistic Regression model: {model_path}")
    else:
