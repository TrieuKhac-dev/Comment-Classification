...
def main():
    parser = argparse.ArgumentParser(
        description="Train LightGBM model with raw data or processed data (mutually exclusive)"
    )
    ...
    if use_processed:
        logger_service.info(f"[Train LightGBM] Using PROCESSED training data file: {train_file}")
    else:
        logger_service.info(f"[Train LightGBM] Using RAW training data file: {train_file}")
    ...
    if use_processed:
        ...
        pipeline = LightGBMPipelineConfig.train_pipeline_with_processed(
        ...
        # Train từ raw data (đầy đủ load + preprocess + split)
